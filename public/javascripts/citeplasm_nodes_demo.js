...
var activate, data, deactivate, edgeIndex, edges, graph, graph_height, graph_tick, graph_width, info, is_edge_connected, is_node_connected, show_info, start_visualization, vertex_radius, vertices, vis;
...
start_visualization = function() {
  ...
  graph = d3.layout.force().charge(-500).linkDistance(vertex_radius * 10).gravity(.1).size([graph_width, graph_height]).nodes(data.vertices).links(data.edges).on("tick", graph_tick).start();
  ...
  vertices.append("svg:circle").attr("class", "vcircle").attr("x", vertex_radius / -2).attr("y", vertex_radius / -2).attr("r", vertex_radius).attr("title", function(v) {
    return v.label;
  });
  vertices.append("svg:text").attr("class", "vlabel").attr("dx", vertex_radius + 5).attr("dy", 5).text(function(v) {
    return v.label;
  });
